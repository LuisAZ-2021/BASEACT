...
namespace Base
{
    public partial class submenuMostrarIndirecto : Form
    {
        public submenuMostrarIndirecto()
        {
            InitializeComponent();
        }
        ...
        public void MostrarIndirecto()
        {
            ...
            try
            {
                using (SqlConnection conector = new SqlConnection(conString))
                {
                    ...
                    I.[% COBERTURA POR USUARIO (AGUA)], I.[% COBERTURA POR USUARIO (CLOACA)], I.[% COBERTURA POR USUARIO (AyC)], 
                    I.[% MICROMEDICION],
                    ...
                    {
                        column.MinimumWidth = 110; // Cambia este valor según tus necesidades (por ejemplo, 5cm)
                    }
                    conector.Close();
                }
            }
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error al mostrar los datos Indirectos:" + ex.Message);
            }
        }
        private void submenuMostrarIndirecto_Load(object sender, EventArgs e)
        {
            // Actualizamos las de Datos Indirectos mediante las formulas
        ...
        private void btnDescargarExcel_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    saveFile.FileName = string.Format("DatosIndirectos_{0}.xlsx", DateTime.Now.ToString("ddMMyyyyHHmm"));
