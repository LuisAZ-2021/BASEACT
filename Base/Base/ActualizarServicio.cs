...
namespace Base
{
    public partial class ActualizarServicio : Form
    {
        ...
        public void MostrarServicio()
        {
            ...
            try
            {
                using (SqlConnection conector = new SqlConnection(conString))
                {
                    ...
                    string query = @"SELECT S.ID, L.LOCALIDADES, S.[TIPO DE SERVICIO], S.[VOLUMEN DISTRIBUIDOS m3/D], S.[HS DE SERV], S.[FACTURA SI/NO], S.[DOTACION ACTIVA],
		            S.[AGUA (CON CONEXION - SERVICIO NO MEDIDO)], S.[CLOACA (CON CONEXION - SERVICIO NO MEDIDO)], S.[AG Y CL (CON CONEXION - SERVICIO NO MEDIDO)],
		            S.[AGUA (SIN CONEXION - radio servido)], S.[CLOACA (SIN CONEXION - radio servido)], S.[AG Y CL (SIN CONEXION - radio servido)],
		            S.[TOTAL TASA BASICA], S.[AGUA (SERVICIO MEDIDO - medidores)], S.[AGUA Y CLOACA (SERVICIO MEDIDO - medidores)], S.[TOTAL SERVICIO MEDIDO],
        ...
        private void InsertChangeRecord(int cellID, int columna, int userID, DateTime changeDate, string comment, string valorCelda)
        {
            ...
            {
                connection.Open();
                string insertQuery = "INSERT INTO Cambios_DatosServicio (idCell, Columna, ValorCelda, Usuario, Fecha, Comentario) " +
